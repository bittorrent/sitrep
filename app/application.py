...
class Application(flask.Flask):
    def __init__(self):
        ...
        self.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://{}{}@{}/{}'.format(
            self.settings['database'].get('user', 'root'),
            (':' + self.settings['database']['password']) if 'password' in self.settings['database'] else '',
            self.settings['database'].get('host', '127.0.0.1'),
...
@application.route('/api/v1/components/<component>/update', methods=['POST'])
